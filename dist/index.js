...
function getMostCarriedCalories(elfData) {
    let mostCalories = 0;
    ...
    console.log(`Elf ${mostCaloriesIndex + 1} is carrying ${mostCalories} Calories (most calories of all).`);
}
// getMostCarriedCalories(data);
// Logs the amount of calories of the top three carrying-elves
function getTopThreeElfCalorieSum(elfData) {
    let values = [];
    for (let i = 0; i < elfData.length; i++) {
        let arraySum = 0;
        for (let y = 0; y < elfData[i].length; y++) {
    ...
    values.sort((a, b) => b - a);
    console.log(`The top three elves carry a total of ${values[0] + values[1] + values[2]} calories.`);
}
